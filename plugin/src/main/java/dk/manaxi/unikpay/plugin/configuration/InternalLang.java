...
public class InternalLang {
    ...
    public void send(Audience audience, String key, TagResolver... args) {
        List<Component> messages = getList(key, args);
        if(messages == null) {
            return;
        }
        if (messages.isEmpty()) {
    ...
    public void load() throws IOException {
        InputStream lang = Main.getInstance().getResource("internallang.yml");
        Callable<BufferedReader> source = () -> new BufferedReader(new InputStreamReader(lang));
        final YamlConfigurationLoader loader = YamlConfigurationLoader.builder().source(source).build();
